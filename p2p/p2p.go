...
import (
	...
	"github.com/Samar2170/bitTorrent-client/client"
...
func (t *Torrent) startDownloadWorker(peer peers.Peer, workQueue chan *pieceWork, results chan *pieceResult) {
	...
	if err != nil {
		log.Printf("error connecting to peer %s: %s", peer, err)
		return
	}
	defer c.Conn.Close()
	...
	for pw := range workQueue {
		...
		if err != nil {
			log.Printf("error downloading piece %d from peer %s: %s", pw.index, peer, err)
			workQueue <- pw
			continue
		}
...
func (t *Torrent) Download() ([]byte, error) {
	...
	for donePieces < len(t.PieceHashes) {
		...
		log.Printf("downloaded piece %d/%d (%.2f%%) from %d workers", donePieces, len(t.PieceHashes), percent, numWorkers)
	}
	close(workQueue)
	return buf, nil
}
